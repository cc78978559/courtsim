...
from courtsim.analysis.nba_real_rosters import ESPN_TEAM_NAMES, build_nba_real_roster_teams
...
def test_real_rosters_preserve_identity_and_balance_rotation_minutes() -> None:
    ...
    assert len(teams) == 30
    assert len({player_id for team in teams for player_id in team.roster_order}) == 300
    assert all(len(team.roster_order) == 10 for team in teams)
